...
class GameCreateView(LoginRequiredMixin, DetailView):
    ...
    def get_object(self):
        # Note: the get_object method is called many times, so
        # care with the game instantiation must be taken.
        # Retrieves the parent Questionnaire
        questionnaire_id = self.kwargs.get("questionnaireid")
        questionnaireobj = get_object_or_404(Questionnaire,
                                             id=questionnaire_id)

        # Check if the game had been created
        if self.game:
            # The object had already been created (get_object
            # was already called)
            self.request.session['game_id'] = self.game.id
            self.request.session['game_state'] = self.game.state
            return self.game

        # Instantiates the Game with the given Questionnaire
        new_game = Game.objects.create(questionnaire=questionnaireobj)
        # Saves the game id and state in a session variable for later
        self.request.session['game_id'] = new_game.id
        self.request.session['game_state'] = new_game.state

        # Sets the game attribute (in case the
        # get_object method is called again)
        self.game = new_game
        return new_game
...
class GameCountDownTime(LoginRequiredMixin, TemplateView):
    ...
    def get_game_state(self):
        ...
        try:
            return self.request.session['game_state']
        except KeyError:
            return models.constants.WAITING
    ...
    def get_template_names(self):
        ...
        if game_state == models.constants.WAITING:
            if len(gameobj.questionnaire.question_set.all()) == 0:
                ...
                st = models.constants.LEADERBOARD
                self.request.session['game_state'] = st
            else:
                # Navigates to FIRST QUESTION (default qNo is already 0)
                self.request.session['game_state'] = models.constants.QUESTION
            # Adds the template
            templates = ['game_count_down.html']
        elif game_state == models.constants.QUESTION:
            # Modifies the state to the question's ANSWER
            self.request.session['game_state'] = models.constants.ANSWER
            # Adds the template
