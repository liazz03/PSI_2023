...
def populate():
    ...
    for bo in books:
        ...
        aut  = Author.objects.filter(first_name__contains=a_fn,
        ...
        # A first save is convenient to avoid
        # Foreign Key problems with the N-M relation
        new_book.save()
        for ge in g:
    ...
    for bi in book_instances:
        ...
        if not bi['due_back']:
            ...
            db = bi['due_back']
        new_book_instance = BookInstance(book=bok, imprint=bi['imprint'],
                                         due_back=db, status=bi['status'])
